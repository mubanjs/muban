...
const getJsonContent = simpleMemo((element: HTMLElement): Record<string, unknown> => {
  // get props json script tag
  // TODO: use ":scope >" to only select direct descendant
  const scriptElement = element.querySelector<HTMLScriptElement>('script[type="application/json"]');
